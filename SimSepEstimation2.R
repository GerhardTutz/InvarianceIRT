...
for (s in 1:numsim){
  ...
  ##### data generation (in particular choice of person parameters)
  ...
  ### noncentral1
  #pers <- rnorm(P, mean = 1, sd = 1)   #### person par
    #pers<-pers^2
  
  ### noncentral2
    #pers <- rnorm(P, mean = 1.5, sd = 1)   #### person par
    #pers<-pers^2  
  ...
  if(length(res.rasch$etapar) == I1) {numsimcond<-numsimcond +1
   # unconditsim[numsimcond,]<-c(0,res.rasch$etapar)
    Abserr[s,3] <-   sum(abs(c(0,res.rasch$etapar)-it))/I 
                                     
    #if(length(res.rasch$etapar) < I1) failcond<-failcond+1
    if(length(res.rasch$etapar) < I1) findfailedcond[s,1]<-1
    }
  #cols<-colSums(resp)
  #for(itc in 1:I) if(cols[itc]==P)indfailedcond[s,1]<-1
  }
...
validcond<-0
for(s in 1:numsim) {if (indfailedcond[s,1]==0) validcond<-c(validcond,s)}
numsucced<-numsim-colSums(indfailedcond)

Error[3]<-sum(Abserr[validcond,3])/numsucced  ### only valid for conditional
...
boxplot(itsimpoly,cex.axis=1.5,cex.lab=1.5,
        cex.main=1.5, main ="Pairwise separation estimates",xlab ="Items", type="b",
        ylab="") #,names=c("1", "2", "3","4","5","6"))
lines(dum,it,pch=16,type='p',cex=1.3)
...
###################
###pairs of estimates with itsimpoly
selitem<-5## selected item
numplot<-40

### separation versus conditional 
wid=1.3
xlims<-c(it[selitem]-wid,it[selitem]+wid)
ylims=xlims

plot(itsimpoly[1:numplot,selitem],unconditsim[1:numplot,selitem], xlab ="Pairwise separation estimates",main ="Item 5",cex.main=1.5,
     ylab="Conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
points(it[selitem],it[selitem], pch = 19,cex=1.5,lwd=2)


### separation versus pairwise conditional

plot(itsimpoly[1:numplot,selitem],itsimpair[1:numplot,selitem],xlab ="Pairwise separation estimates",main ="Item 5",cex.main=1.5,
     ylab="Pairwise conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
points(it[selitem],it[selitem], pch = 19,cex=1.5,lwd=2)


### conditional versus pairwise conditional

plot(unconditsim[1:numplot,selitem],itsimpair[1:numplot,selitem],xlab ="Conditional estimates",main ="Item 5",cex.main=1.5,
     ylab="Pairwise conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
...
###############################################
###pairs of estimates with itsim
selitem<-4## selected item
numplot<-40
...
plot(itsim[1:numplot,selitem],unconditsim[1:numplot,selitem], xlab ="Pairwise separation estimates",main ="Item 4",cex.main=1.5,
     ylab="Conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
points(it[selitem],it[selitem], pch = 19,cex=1.5,lwd=2)
...
plot(itsim[1:numplot,selitem],itsimpair[1:numplot,selitem],xlab ="Pairwise separation estimates",main ="Item 4",cex.main=1.5,
     ylab="Pairwise conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
points(it[selitem],it[selitem], pch = 19,cex=1.5,lwd=2)
...
plot(unconditsim[1:numplot,selitem],itsimpair[1:numplot,selitem],xlab ="Conditional estimates",main ="Item 4",cex.main=1.5,
     ylab="Pairwise conditional estimates",cex.axis=1.5,cex.lab=1.5,cex=1.5,lwd=2, xlim=xlims,ylim=ylims)
lines(c(-5,5),c(-5,5))
